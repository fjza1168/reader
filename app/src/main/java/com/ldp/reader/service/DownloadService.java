...
import com.ldp.reader.ui.base.BaseService;
...
public class DownloadService extends BaseService {
    ...
    public int onStartCommand(Intent intent, int flags, int startId){
        ...
        Disposable disposable = RxBus.getInstance()
                ...
                .subscribe(
                        new Consumer<DownloadTaskBean>() {
                            @Override
                            public void accept(DownloadTaskBean event) throws Exception {
                                //判断任务是否为轮询标志
                                //判断任务是否存在，并修改任务
                                if (TextUtils.isEmpty(event.getBookId())
                                        || !DownloadService.this.checkAndAlterDownloadTask(event)) {
                                    DownloadService.this.addToExecutor(event);
                                }
                                Log.d("+任务", event.getBookId()+"书名");
                            }
                        }
                );
        ...
        for (DownloadTaskBean downloadTask : mDownloadTaskList){
            ...
            if (downloadTask.getStatus() == DownloadTaskBean.STATUS_FINISH){
                ...
                //判断，是否已完成的章节的起始点比新Task大，如果更大则表示新Task中的该章节已被加载，所以需要剪切
                        ...
                        && newTask.getBookChapterList().size() >  downloadTask.getLastChapter()
                        && newTask.getBookChapterList().size() >= newTask.getLastChapter()){
                    //删除掉已经完成的章节
                    Log.d("+剪切错误",newTask.getBookChapterList().size() + "起始  " + downloadTask.getLastChapter() + " 结束" +  newTask.getLastChapter());
